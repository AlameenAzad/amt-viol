<template>
  <q-table :data="data" :columns="columns" row-key="title" :filter="query"
    class="bg-white shadow-1 radius-20 -mb-xl pagination-no-shadow" :pagination="{
      sortBy: 'title',
      descending: true,
      page: 1,
      rowsPerPage: 15
    }" :loading="loading" color="primary" :rows-per-page-label="$t('Records per page')">
    <template v-slot:top>
      <div class="col-12 col-md-3">
        <q-input borderless outlined class="input-radius-6 no-shadow q-mb-sm q-mt-sm" v-model="query"
    ...
    <template v-slot:header="props">
      <q-tr :props="props">
        <q-th v-for="col in props.cols" :key="col.name" :props="props" class="text-blue font-14">
          {{ col.label }}
        </q-th>
      </q-tr>
    </template>
