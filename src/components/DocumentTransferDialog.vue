<template>
    <q-dialog v-model="$_options">
      <q-card class="q-px-xl radius-10 column" style="display: flex; justify-content: center; height: 350px; position: absolute; top: 50px;">
      ...
      async transferDocument() {
        ...
        try {
          try {
            ...
            this.$q.notify({
              message:
                "Das Dokument wurde erfolgreich übertragen",
              type: "positive"
            });
            this.$router.go(-1);
          } catch (error) {
